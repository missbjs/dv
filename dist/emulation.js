...
export const VIEWPORT_METRICS_EXPRESSION = 'JSON.stringify([innerWidth,innerHeight,outerWidth,outerHeight,devicePixelRatio])';
/**
 ...
 */
export function detectEmulatedViewport(m) {
    const base = {
        ...m,
        emulated: false,
        reason: undefined,
    ...
    if (!m.outerWidth || !m.outerHeight || !m.innerWidth || !m.innerHeight) {
        const zeroed = ['innerWidth', 'innerHeight', 'outerWidth', 'outerHeight'].filter((k) => !m[k]);
        return {
            ...base,
            emulated: false,
            conclusive: false,
 ...
 */
export function sessionScopedNote(what) {
    return [
...
export function parseViewportMetrics(value) {
 ...
 */
export async function probeViewport(port, tabId, timeoutMs = 2500) {
