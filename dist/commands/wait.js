...
async function waitForNetworkIdle(client, maxWait) {
    const idleTime = 500;
...
async function waitForSelector(client, selector, maxWait) {
    ...
    while (Date.now() < deadline) {
        try {
            const result = await client.evaluate(`!!document.querySelector('${selector.replace(/'/g, "\\'")}')`);
...
async function waitForRef(client, ref, maxWait) {
    const pollInterval = 200;
    const deadline = Date.now() + maxWait;
    while (Date.now() < deadline) {
        try {
...
async function waitForText(client, text, maxWait) {
    const pollInterval = 200;
    const deadline = Date.now() + maxWait;
