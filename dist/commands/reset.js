...
export async function reset(options) {
    ...
    try {
        ...
        const failed = tabResults.flatMap((t) => t.results.filter((r) => !r.cleared).map((r) => ({ ...r, tab: t.id })));
        ...
        console.log(chalk.blue((targets.length === CLEARERS.length ? 'Resetting emulation state' : 'Resetting selected emulation state') + scope + '...'));
...
async function contentTabs(port) {
    const client = new CDPClient(port);
