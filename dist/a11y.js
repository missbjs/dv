 ...
 */
export function auditA11y(nodes, refMap, // nodeId → ref
url) {
    const issues = [];
    const nodeMap = new Map();
    const childMap = new Map(); // parentId → childIds (for non-ignored children)
    ...
    for (const node of visibleNodes) {
        ...
        if (isInput) {
            ...
            if (!hasLabel && !hasLabelledby) {
                ...
                issues.push(issue(refMap, node, sev, sev === 'ERROR' ? 'Missing label' : 'No label (placeholder only)', 'missing-label'));
            }
        }
    }
    ...
    for (let i = 0; i < headings.length; i++) {
        ...
        if (i > 0) {
            ...
            if (prevLevel >= 1 && currLevel > prevLevel + 1) {
                issues.push(issue(refMap, curr, 'WARN', `Heading skip: h${prevLevel} → h${currLevel}`, 'heading-skip'));
            }
        }
    }
    ...
    for (const node of visibleNodes) {
        if (hasProperty(node, 'hidden') && hasProperty(node, 'focusable')) {
            issues.push(issue(refMap, node, 'ERROR', 'aria-hidden element is focusable', 'aria-hidden-focusable'));
        }
    }
    ...
    for (const node of visibleNodes) {
        if (node.role?.value === 'button') {
            if (!node.name?.value || node.name.value.trim() === '') {
                issues.push(issue(refMap, node, 'WARN', 'Button has no accessible name', 'empty-button'));
            }
        }
    }
    // ── Check 6: Empty links ──
    for (const node of visibleNodes) {
        if (node.role?.value === 'link') {
            if (!node.name?.value || node.name.value.trim() === '') {
                issues.push(issue(refMap, node, 'ERROR', 'Link has no accessible name', 'empty-link'));
            }
        }
    }
    ...
    for (const node of visibleNodes) {
        if (hasProperty(node, 'invalid')) {
            issues.push(issue(refMap, node, 'WARN', 'Element marked as invalid — may indicate accessibility issue', 'invalid-element'));
        }
    }
    // Count severities
...
function issue(refMap, node, severity, message, rule) {
    return {
 ...
 */
export function formatA11yReport(report) {
    const { issues, summary } = report;
    const out = [];
    out.push(chalk.bold('Accessibility Audit'));
    out.push('='.repeat(50));
    out.push(chalk.gray(`${issues.length} issue(s) (${summary.errors} errors, ${summary.warnings} warnings, ${summary.infos} info) ` +
    ...
    for (const sev of sevOrder) {
        ...
        if (group.length > 0) {
            ...
            for (const issue of group) {
                out.push(sevColor(`  ${issue.severity}  ${chalk.cyan(issue.ref)} ` +
