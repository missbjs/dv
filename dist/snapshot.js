 ...
 */
export function extractFlags(node) {
    const flags = [];
    if (!node.properties)
        return flags;
        ...
        switch (prop.name) {
            ...
            case 'checked':
                if (v.value !== undefined && v.value !== false)
                    flags.push(`checked=${v.value}`);
                break;
            case 'expanded':
                if (v.value !== undefined)
                    flags.push(v.value ? 'expanded' : 'collapsed');
                break;
            case 'selected':
                if (v.value === true)
                    flags.push('selected');
            ...
            case 'level':
                if (v.value !== undefined)
                    flags.push(`level=${v.value}`);
                break;
            case 'hasPopup':
                if (v.value !== undefined && v.value !== false)
                    flags.push(`hasPopup=${v.value}`);
                break;
            case 'pressed':
                if (v.value !== undefined && v.value !== false)
                    flags.push(`pressed=${v.value}`);
                break;
            case 'orientation':
                if (v.value === 'vertical' || v.value === 'horizontal')
                    flags.push(v.value);
                break;
        }
    }
 ...
 */
export function buildSnapshotLines(nodes, existingRefs) {
    const nodeMap = new Map();
    const rootNodes = [];
    const usedNewRefs = new Map(); // backendDOMNodeId → newly assigned ref
    ...
    const lines = [];
    const refs = new Map();
    const backendNodeMap = new Map(); // backendDOMNodeId → ref
    // Track the next sibling index for each parent ref
    const siblingCounters = new Map();
    function getNextRef(parentRef) {
    ...
    function dfs(node, parentRef, depth) {
 ...
 */
export async function anchorRefs(client, nodes) {
    ...
    for (let i = 0; i < orderedNodes.length; i += BATCH_SIZE) {
        ...
        for (let j = 0; j < batch.length && j < nodeIds.length; j++) {
            ...
            if (!nodeId || node.backendDOMNodeId === undefined)
                continue;
            try {
 ...
 */
export function resolveRef(ref, result) {
    return result.refs.get(ref) ?? null;
}
/**
 * Format snapshot lines as a displayable text tree.
 */
export function formatSnapshotLines(result) {
    const out = [];
