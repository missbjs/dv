...
import { probeViewport, ViewportReport } from '../emulation.js';
...
export async function status(options: StatusOptions) {
  ...
  try {
    ...
    const viewports: Array<ViewportReport | null> = probeViewports
      ? await Promise.all(
          tabs.map(tab =>
            tab.url.startsWith('devtools://') ? Promise.resolve(null) : probeViewport(port, tab.id)
          )
        )
      : tabs.map(() => null);
    ...
    if (tabs.length > 0) {
      ...
      if (inconclusiveCount + unprobedCount > 0) {
        ...
        console.log(chalk.yellow(
          `⚠ ${inconclusiveCount + unprobedCount} tab(s) could not be checked for an emulated viewport.`
        ));
        console.log(chalk.gray(
          `  Bring the tab to the front (${options.profile} select <id>) and re-run status;`
        ));
...
function vpReason(viewports: Array<ViewportReport | null>): string {
 ...
 */
const HIDDEN_TAB_CAVEAT =
  'A background tab can keep reporting the old size until it is next in front.';
...
export function clearHints(
  ...
  viewports: Array<ViewportReport | null>
): string[] {
