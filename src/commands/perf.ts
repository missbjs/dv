...
export async function perf(options: PerfOptions) {
  ...
  try {
    ...
    const rows = metricsList.map((m: any) => {
    ...
    for (const [groupName, keys] of Object.entries(groups)) {
      const groupMetrics = rows.filter((r: any) => keys.includes(r.name));
    ...
    const ungrouped = rows.filter((r: any) => !allGrouped.has(r.name));
