...
export async function diff(options: DiffOptions) {
  ...
  if (options.compare) {
    ...
    try {
      ...
      const currentRefs: Record<string, any> = {};
...
function computeSnapshotDiff(
  refs1: Record<string, any>,
  refs2: Record<string, any>,
): SnapshotDiff {
  const keys1 = new Set(Object.keys(refs1));
  const keys2 = new Set(Object.keys(refs2));

  const added: RefEntry[] = [];
  const removed: RefEntry[] = [];
  const changed: Array<{ ref: string; before: RefEntry; after: RefEntry; changes: string[] }> = [];
  let unchanged = 0;

  // Find removed (in refs1 but not refs2)
  for (const key of keys1) {
    if (!keys2.has(key)) {
      removed.push(refs1[key] as RefEntry);
    }
  }
  ...
  for (const key of keys2) {
    if (!keys1.has(key)) {
      added.push(refs2[key] as RefEntry);
    }
  }
  ...
  for (const key of keys1) {
    if (keys2.has(key)) {
      ...
      if (a.role !== b.role) diffs.push(`role: ${a.role} → ${b.role}`);
      if (a.name !== b.name) diffs.push(`name: "${a.name}" → "${b.name}"`);
      if (a.value !== b.value) diffs.push(`value: ${a.value} → ${b.value}`);
      ...
      if (diffs.length > 0) {
        changed.push({ ref: key, before: a as RefEntry, after: b as RefEntry, changes: diffs });
      } else {
        unchanged++;
      }
    }
  ...
  // Sort by ref
  const sortRef = (a: RefEntry, b: RefEntry) => compareRefs(a.ref, b.ref);
  added.sort(sortRef);
...
function printDiff(diff: SnapshotDiff) {
